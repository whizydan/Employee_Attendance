...
<%
    ...
    ResultSet results = db.executeQuery("SELECT attendance.id,attendance.date,attendance.confidence_score,attendance.staffId,attendance.clock,users.id as userId,users.name,users.email from attendance inner join users on attendance.staffId = users.id");
    while(results.next()){
        Users user = new Users();
        user.setName(results.getString("name"));
        user.setEmail(results.getString("email"));
        user.setId(results.getInt("staffId"));
        user.setCreated_on(results.getString("date"));
        user.setPassword(results.getString("clock"));
        user.setConfidence(results.getDouble("confidence_score"));
        users.add(user);
    }
%>
<!DOCTYPE html>
...
<li><a class="active" href="attendancelist.jsp">Attendance List</a></li>
...
<div class="page-btn">
<a href="attendance.jsp" class="btn btn-added"><img src="assets/img/icons/plus.svg" class="me-2" alt="img">Record Attendance</a>
</div>
</div>
...
<li>
<li>
<a data-bs-toggle="tooltip" onClick="onClick();" data-bs-placement="top" title="print"><img src="assets/img/icons/printer.svg" alt="img"></a>
</li>
</ul>
</div>
</div>
...
<%
    if(item.getPassword().equals("in")){
%>
<td><span class="badges bg-lightgreen"><%=item.getPassword()%></span></td>
<%
    }else{
%>
<td><span class="badges bg-lightred"><%=item.getPassword()%></span></td>
<%
    }%>
<%
    if(item.getConfidence() > 50.0){
%>
<td><span class="badges bg-lightgreen"><%=item.getConfidence()%></span></td>
<%
    }else{
%>
<td><span class="badges bg-lightred"><%=item.getConfidence()%></span></td>
<%
    }%>
<td hidden>
<a class="me-3" href="createexpense.jsp?id=<%=item.getId()%>">
<img src="assets/img/icons/edit.svg" alt="img">
</a>
<a class="me-3" href="deleteuser.jsp?id=<%=item.getId()%>">
