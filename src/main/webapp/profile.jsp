...
<%
    ...
    String emailOne = (String) new SessionManager().getSessionAttribute(request,"email");
    String query = "select * from admin where email='"+emailOne+"'";
    ResultSet result = db.executeQuery(query);
    String id = "",name="",phone="",password="",createdOn="";
    while(result.next()){
        id = result.getString("id");
        name = result.getString("name");
        phone = result.getString("status");
        if(phone.equals("0")){phone="Enabled";}else{phone="Disabled";}
    ...
    if (request.getMethod().equalsIgnoreCase("post")) {
        ...
        String queryUpdate = "update admin set email='"+email+"',password='"+password+"',name='"+name+"' where email='"+emailOne+"'";
        SessionManager sess = new SessionManager();
        sess.createSession(request,"email",email);
...
<div class="form-group">
<label>Email</label>
<input name="email" required type="text" value="<%=emailOne%>">
</div>
</div>
<div class="col-lg-6 col-sm-12">
<div class="form-group">
...
<input readonly type="text" value="<%=createdOn%>">
</div>
</div>
<div class="col-lg-6 col-sm-12">
<div class="form-group">
...
<input name="password" required type="password" class=" pass-input" value="<%=password%>">
