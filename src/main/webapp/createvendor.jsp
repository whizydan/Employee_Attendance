...
<%
    String user = (String)new SessionManager().getSessionAttribute(request,"email");
    DatabaseManager db = new DatabaseManager();
    List<Products> productsExpired = db.getExpiredProducts();
    List<Products> fewProducts = db.getFewProducts();
    String id = request.getParameter("id");
    String firstName="",firstCountry="",firstEmail="", firstPhone="";
    int role = 1;
    if(id != null){
        ResultSet set = db.executeQuery("select * from vendors where id = '"+id+"'");
        while(set.next()){
        firstName = set.getString("name");
        firstCountry = set.getString("country");
        firstEmail = set.getString("email");
        firstPhone = set.getString("phone");
        }
    }
    ...
    if(id != null){
        ...
        query = "UPDATE `vendors` SET `name`='"+name+"',"
        + "`phone`='"+phone+"',`email`='"+email+"',`country`='"+country+"' WHERE id='"+id+"'";
    }
    else{
        query = "INSERT INTO `vendors`(`name`, `phone`, `email`, `country`) "
...
<li><a class="active" href="vendorlist.jsp">Vendor list</a></li>
...
<label>Date the member joined</label>
<div class="input-groupicon">
<input value="<%=firstCountry%>" name="country" required type="text" placeholder="Country">
</div>
</div>
</div>
<div class="col-lg-3 col-sm-6 col-12">
...
<div class="input-groupicon">
    <input placeholder="Full name" value="<%=firstName%>" required name="name" type="text">
</div>
</div>
</div>
<div class="col-lg-3 col-sm-6 col-12">
<div class="form-group">
<label>Email</label>
<input placeholder="Email" value="<%=firstEmail%>" name="email" required type="text">
</div>
</div>
<div class="col-lg-12">
<div class="form-group">
<label>Phone</label>
<input required value="<%=firstPhone%>" placeholder="07xxxxxxx" name="phone" type="text">
</div>
</div>
<div class="col-lg-12">
<input type="submit" class="btn btn-submit me-2">
<a href="vendorlist.jsp" class="btn btn-cancel">Cancel</a>
</div>
</div>
</div>
</div>
